...
const subjects=['Solicitar apresentação executiva','Falar com Relações Institucionais','Integração e interoperabilidade','Gestores Públicos','Inteligência Epidemiológica','Redes Hospitalares','Outros assuntos'];
const subjectByParam:Record<string,string>={apresentacao:subjects[0],relacoes:subjects[1],integracao:subjects[2],'gestao-publica':subjects[3],epidemiologia:subjects[4],'saude-privada':subjects[5]};
const whatsappPattern='(?:\\+?55\\s?)?\\(?[1-9]\\d\\)?\\s?9\\d{4}[-\\s]?\\d{4}';

export default function Contato(){
  const [params]=useSearchParams();
  const requestedSubject=params.get('assunto')||'';
  const [selectedSubject,setSelectedSubject]=useState(subjectByParam[requestedSubject]||subjects[0]);
  ...
  useEffect(()=>{setSelectedSubject(subjectByParam[requestedSubject]||subjects[0]);},[requestedSubject]);

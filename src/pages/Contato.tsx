...
import {appendLocation,namePattern,normalizePhone,phonePattern,sendContact} from '../utils/contactForm';
...
export default function Contato(){
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=event.currentTarget;if(!form.reportValidity())return;if(!token){setError('Confirme o reCAPTCHA antes de enviar.');setStatus('error');return}setStatus('sending');setError('');try{const body=new FormData(form);body.set('telefone',normalizePhone(String(body.get('telefone')??'')));body.append('recaptchaToken',token);appendLocation(body,location);await sendContact(body);form.reset();setStatus('success');setResetKey(value=>value+1)}catch(caught){setError(caught instanceof Error?caught.message:'Não foi possível enviar a mensagem. Tente novamente.');setStatus('error');setResetKey(value=>value+1)}};
  return <Box sx={{py:{xs:8,md:12},bgcolor:'#f8f9fa'}}><Container><Grid container spacing={7}><Grid item xs={12} md={5}><Typography variant="overline" color="#d81159" fontWeight={800}>CONTATO</Typography><Typography variant="h1" sx={{mt:2}}>Como podemos ajudar?</Typography><Typography color="text.secondary" mt={3} fontSize={17}>Escolha o assunto e envie sua mensagem. Nossa equipe responderá pelo e-mail informado.</Typography></Grid><Grid item xs={12} md={7}><Box component="form" onSubmit={submit} noValidate sx={{bgcolor:'#fff',border:'1px solid #e0e0e0',p:{xs:3,md:5}}}><Grid container spacing={2}>
    ...
    <Grid item xs={12} sm={6}><TextField required fullWidth name="nome" label="Nome completo" autoComplete="name" inputProps={{minLength:5,maxLength:120,pattern:namePattern.source}} helperText="Informe nome e sobrenome."/></Grid>
    ...
    <Grid item xs={12} sm={6}><TextField required fullWidth name="telefone" type="tel" label="Telefone - WhatsApp" autoComplete="tel" inputProps={{inputMode:'tel',pattern:phonePattern.source,minLength:10,maxLength:20}} helperText="DDD + número; inclua o país se necessário."/></Grid>
